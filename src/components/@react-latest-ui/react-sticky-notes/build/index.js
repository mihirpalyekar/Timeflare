...
function getNotes(colorCodes, notes) {
  ...
  } else if (localStorage.getItem('react-sticky-not')) {
    _notes = JSON.parse(localStorage.getItem('react-sticky-not'));
    fetch("http://localhost:3000/api/user/user").then((response) => response.json()).then(data=>{
      console.log(data)
      localStorage.setItem("checking", JSON.stringify(data.data))
    })
